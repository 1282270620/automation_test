...
class CoachingLeaderScores_L2Default(unittest.TestCase):


    def setUp(self):
        #Case ID
        self.caseID="CoachingLeaderScores_L2Default"
        ...
        self.sql_TLs_ByL2="Sql_TLs_ByL2_LeaderAcademyCoachingLeaderScorespage"
        #self.sql_TLs_ByL3="Sql_TLs_ByL3_LeaderAcademyCoachingLeaderScorespage"
        self.sql_TLsValue_ByL2="Sql_KPIValueOfTLs_ByL2_LeaderAcademyCoachingLeaderScorespage"
        self.sql_AVGSCOREOFFORM_ByL2="Sql_AVGSCOREOFFORM_ByL2_LeaderAcademyCoachingLeaderScorespage"
    ...
    def test_CoachingLeaderScores_L2Default(self):
            ...
            if Flag==True:
                ...
                if lobname in self.LOBs_NoLSACS_list:
                    ...
                    for n in range(0,len(site_list)):
                        ...
                        L2Info=GetL2.get_TLandAgentInfofromAdmin_Byrole(Adminurl, lobname, sitename, self.OMuserid, self.OMpassword, "L2")
                        L2Hrid=L2Info["Hrid"]
                        ...
                        sql_TLs_ByL2=GetConfig.get_sql_content(self.sqlsheetname, self.sql_TLs_ByL2).replace("L2Hrid",L2Hrid)
                        AllTLsInfo_DataBase=HMysql.Get_datafromDB(hostindex, lobname_database, sitename_database, self.dbuser, self.dbpassword, sql_TLs_ByL2)
                        ...
                        Sql_TLsValue_ByL2=GetConfig.get_sql_content(self.sqlsheetname, self.sql_TLsValue_ByL2).replace("DateType",DateType).replace("LastSixMonth_Date",LastSixMonth_Date_need).replace("Current_Date",Current_Date_need).replace("L2Hrid",L2Hrid)
                        AllTLsKPIinf_DataBase=HMysql.Get_datafromDB(hostindex, lobname_database, sitename_database, self.dbuser, self.dbpassword, Sql_TLsValue_ByL2)
                        ...
                        DateType="acknowledge"
                        sql_AVGSCOREOFFORM_ByL2=GetConfig.get_sql_content(self.sqlsheetname, self.sql_AVGSCOREOFFORM_ByL2).replace("DateType",DateType).replace("LastSixMonth_Date",LastSixMonth_Date).replace("Current_Date",Current_Date)
                        AllAVGSCOREOFFORM=HMysql.Get_datafromDB(hostindex, lobname_database, sitename_database, self.dbuser, self.dbpassword, sql_AVGSCOREOFFORM_ByL2)
