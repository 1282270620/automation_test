...
#import MySQLdb
from CoachingAndTriadCoaching_Pages.SalesQuickHitForm import SalesQuickHit
...
class SalesQuickHitForm_PlannedToAcknowledged(unittest.TestCase):


    def setUp(self):
        #Case ID
        self.caseID="SalesQuickHitForm_PlannedToAcknowledged"
        ...
        #Coachformname
        self.coachformname="Sales Quick Hit Form" 
        ...
        #ObservationReason status & RadioButton status
        self.ObservationReasonAndRadioButton_CheckedStatus='fa fa-check-circle radio-btn checked'
        self.ObservationReasonAndRadioButton_UnCheckedStatus='fa fa-check-circle radio-btn'
    ...
    def test_SalesQuickHitForm_PlannedToAcknowledged(self):
        ...
        Header=HeaderPage()
        SalesQuickHitForm=SalesQuickHit()
        ...
        for i in range(0,len(self.testLOBSITE_list)):
            ...
            if Flag==True:
                ...
                for n in range(0,len(site_list)):
                    ...
                    '''Check2:1.Page title :'Coaching - Commendation'.
                    ...
                    #Datalist_49=HMysql.Get_datafromDB(49, lobname, sitename, self.dbuser, self.dbpassword, sql_coach)
                    #Datalist_50=HMysql.Get_datafromDB(49, lobname, sitename, self.dbuser, self.dbpassword, sql_coach)
                    #Datalist_51=HMysql.Get_datafromDB(49, lobname, sitename, self.dbuser, self.dbpassword, sql_coach)
                    ...
                    '''Check6:Return to Coaching list page,On search fields section,show:
                              ...
                              6.On three databases servers: all info are the same.
                              7.This coach record is shown by default  with  status 'Planned'.'''
                    ...
                    #assert Datalist_49[0]==Datalist_50[0]==Datalist_51[0]#Only for Commendation Form
                    assert Status_Coach=="Planned"
                        
                    #Step3.7:Select this record to enter the  form again. 
                    #CoachHome.click_eachcoach(typename_path)
                    CoachHome.click_eachcoach(1)
                    '''Check7:Enter the  form detail page with title 'Coaching - Sales Quick Hit Form'''
                    assert Header.get_HeaderTittle()==self.coachpagetitle 
                    ...
                    #Step98-1)-3):1.Input Call Recording ID ='callRecordingID_001&002'
                            #2.select ' Date and Time of Call'=select the first date in date picker
                            #3.input call Recording Number
                            #input CID/MEDIA TACTIC
                            #Select the first option of Observation Reason
                    CDMEDIATACTIC=self.Shorttextboxesprefix1+SalesQuickHitForm.get_TacticTitle()
                    Call_Date_Time_Input='2016-11-1 12:00AM'
                    call_Recording_Number=self.CallRecordingNumber1
                    SalesQuickHitForm.input_tactic(CDMEDIATACTIC)
                    SalesQuickHitForm.select_ObservationReason(1)
                    if lobname != "AOL":
                        BasicInfo.input_callrecordingnumber(call_Recording_Number)
                    SalesQuickHitForm.input_CallDateTime(Call_Date_Time_Input)
                    
                    #assert SalesQuickHitForm.get_ObservationReasonStatus(1)==self.ObservationReasonAndRadioButton_CheckedStatus
                    #assert SalesQuickHitForm.get_ObservationReasonStatus(2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    #assert SalesQuickHitForm.get_ObservationReasonStatus(3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    
                    
                    '''Check9:1.'Date and time of Call'  cannot be manually inputted;
                               2.'Date and Time of Call show xxxx-xx-xx xx:xx XX, after selected'''
                    assert SalesQuickHitForm.get_CallDateTime()=="" #Date and time of Call'  cannot be manually inputted.
                    SalesQuickHitForm.select_anyDateTimeOfCall(1,1)
                    time.sleep(30*Gl.waittime)
                    DateandTimeofCall_selected=SalesQuickHitForm.get_CallDateTime()
                    assert DateandTimeofCall_selected!=""#Field 'Date and Time of Call' show time,after selected
                    
                    #step8-4) Click YES and No RADIO BUTTONS
                    b=[2, 3, 4, 6, 7, 8, 10, 11, 12]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionOne(buttonindex,1)
                    b=[2, 3, 4, 5, 7, 8, 9, 11, 12] 
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionTwo(buttonindex,1)
                    b=[2, 3, 5, 6, 7, 9, 10, 11, 12]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionThree(buttonindex,1)
                    b=[2, 3, 4]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionFour(buttonindex, 1)
                    b=[2,3]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionFive(buttonindex, 2)
                        
                    
                    #Step9:Click Save and continue later button
                    ...
                    #Total_coachnumber_database10=cursor.execute(sql_coach)
                    '''Check10:1.Check total number of coachings in tablet is the same with it in database
                               2.On three databases servers: all info are the same.
                               3.Check Status of all coachings in tablet is changed to Ongoing'''
                    assert Total_coachnumber_tablet10==Total_coachnumber_database10 
                    #assert Datalist_49[0]==Datalist_50[0]==Datalist_51[0]#Only for Commendation Form                   
                    assert Status_Coach=="Ongoing"
                    
                    #Step3.11:Select this record to open form detail page again.
                    #CoachHome.click_eachcoach(typename_path)
                    CoachHome.click_eachcoach(1)
                    '''Check11:1.Check the title of the detail page
                               2.Check the status of this coaching in detail page is Ongoing
                               3.Check the vaule of Call RecordingID is the same as that input
                               4.Check the vaule of Date and Time of Call is the same as that selected before
                               5.Check the vaule of Call Recording Number is the same as that input before
                               6.Check the first two and fourth KPIs is in checked status
                               7.Check  Other KPI is shown unchecked status
                               8.check status of radio buttons'''
    
                    assert Header.get_HeaderTittle()==self.coachpagetitle
                    assert BasicInfo.get_status()=="Ongoing"
                    assert SalesQuickHitForm.get_CallDateTime()==DateandTimeofCall_selected
                    print SalesQuickHitForm.get_tactic()
                    print CDMEDIATACTIC
                    assert SalesQuickHitForm.get_tactic()==CDMEDIATACTIC
                    print SalesQuickHitForm.get_ObservationReasonStatus(1)
                    print self.ObservationReasonAndRadioButton_CheckedStatus
                    assert SalesQuickHitForm.get_ObservationReasonStatus(1)==self.ObservationReasonAndRadioButton_CheckedStatus
                    assert SalesQuickHitForm.get_ObservationReasonStatus(2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    assert SalesQuickHitForm.get_ObservationReasonStatus(3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    ...
                    for buttonindex in b:
                        #print buttonindex
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    b=[2, 3, 4, 5, 7, 8, 9, 11, 12] 
                    for buttonindex in b:
                        #print buttonindex
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        
                    b=[2, 3, 5, 6, 7, 9, 10, 11, 12]
                    for buttonindex in b:
                        print buttonindex
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    b=[2, 3, 4]
                    for buttonindex in b:
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    b=[2,3]
                    for buttonindex in b:
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFiveStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFiveStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFiveStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus 
                     
                        
                    #Step3.11:1.In all  other text boxes with following content:Firstly input content in test boxes to test.
                            #2.Click "Save and Continue Later"
                    SalesQuickHitForm.input_FirstThreelongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(4),4)
                    SalesQuickHitForm.input_FirstThreelongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(5),5)
                    SalesQuickHitForm.input_FirstThreelongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(6),6)
                    
                    SalesQuickHitForm.input_SecondThreelongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(7),7,5)
                    SalesQuickHitForm.input_SecondThreelongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(8),8,4)
                    SalesQuickHitForm.input_SecondThreelongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(9),9,8)
                    SalesQuickHitForm.input_Seventhlongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(10))
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(1),2)
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(1),3)
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(4),5)
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(4),6)
                    #step12 click Save and Continue Later button
                    BasicInfo.Click_Button(2)
                    Status_Coach=CoachHome.get_anyCoach_attribute(1, 6)
                    '''Check11:Check the status of this coaching in detail page is Ongoing'''
                    assert Status_Coach=="Ongoing"
                    
                    #Step13:Open this record  form again.
                    #CoachHome.click_eachcoach(typename_path)
                    CoachHome.click_eachcoach(1)
                    '''Check12:1.Check the title of the detail page
                               2.Check the status of this coaching in detail page is Ongoing
                               3.Check the vaule of Call RecordingID is the same as that input
                               4.Check the vaule of Date and Time of Call is the same as that selected before
                               5.Check the vaule of Call Recording Number is the same as that input before
                               6.Check the first two and fourth KPIs is in checked status
                               7.Check  Other KPI is shown unchecked status
                               8.Check the content shown in text box is the same as that input'''
                    ...
                    print SalesQuickHitForm.get_FirstThreelongtextbox(4)
                    print self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(4)
                    assert SalesQuickHitForm.get_FirstThreelongtextbox(4)==self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(4)
                    assert SalesQuickHitForm.get_FirstThreelongtextbox(5)==self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(5)
                    assert SalesQuickHitForm.get_FirstThreelongtextbox(6)==self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(6)
                    print SalesQuickHitForm.get_SecondThreelongtextbox(7, 5)
                    print self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(7)
                    
                    assert SalesQuickHitForm.get_SecondThreelongtextbox(7, 5)==self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(7)
                    assert SalesQuickHitForm.get_SecondThreelongtextbox(8, 4)==self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(8)
                    assert SalesQuickHitForm.get_SecondThreelongtextbox(9, 8)==self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(9)
                    
                    assert SalesQuickHitForm.get_Seventhlongtextbox()==self.Longtextboxesprefix1+SalesQuickHitForm.get_FristSevenlongtextboxTitle(10)
                    
                    assert SalesQuickHitForm.get_LastFourlongtextbox(2)==self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(1)
                    assert SalesQuickHitForm.get_LastFourlongtextbox(3)==self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(1)
                    assert SalesQuickHitForm.get_LastFourlongtextbox(5)==self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(4)
                    assert SalesQuickHitForm.get_LastFourlongtextbox(6)==self.Longtextboxesprefix1+SalesQuickHitForm.get_LastTwolongtextboxTitle(4)
                    
                    
                    #Step14:1.Change content in all editable fields to new one:
                    ...
                    CDMEDIATACTIC_new=self.Shorttextboxesprefix2+SalesQuickHitForm.get_TacticTitle()
                    #Call_Date_Time_Input='2016-11-1 12:00AM'
                    call_Recording_Number=self.CallRecordingNumber2
                    SalesQuickHitForm.input_tactic(CDMEDIATACTIC_new)
                    SalesQuickHitForm.select_ObservationReason(3) #select 'Skills Development'
                    SalesQuickHitForm.select_anyDateTimeOfCall(1,2)#select a new date time
                    #DateandTimeofCall_selected=SalesQuickHitForm.get_CallDateTime()
                    DateandTimeofCall_selected_New=SalesQuickHitForm.get_CallDateTime()
                    call_Recording_Number_new=self.CallRecordingNumber2
                    #Content_InTextBox_new=self.Longtextboxesprefix2+CommendationForm.get_commendationBoxTitle()
                    ...
                    #step14-4) Click YES and No RADIO BUTTONS
                    b=[2, 3, 4, 6, 7, 8, 10, 11, 12]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionOne(buttonindex,2)
                    b=[2, 3, 4, 5, 7, 8, 9, 11, 12] 
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionTwo(buttonindex,2)
                    b=[2, 3, 5, 6, 7, 9, 10, 11, 12]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionThree(buttonindex,2)
                    b=[2, 3, 4]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionFour(buttonindex,2)
                    b=[2,3]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionFive(buttonindex,2)
                    b=[1, 2, 3, 4, 5, 6, 7 ]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionSix(buttonindex, 1)
                    #step14-5  edit text boxes
                    SalesQuickHitForm.input_FirstThreelongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(4),4)
                    SalesQuickHitForm.input_FirstThreelongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(5),5)
                    SalesQuickHitForm.input_FirstThreelongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(6),6)
                    SalesQuickHitForm.input_SecondThreelongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(7),7, 5)
                    SalesQuickHitForm.input_SecondThreelongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(8),8, 4)
                    SalesQuickHitForm.input_SecondThreelongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(9),9, 8)
                    SalesQuickHitForm.input_Seventhlongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(10))
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(1),2)
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(1),3)
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(4),5)
                    SalesQuickHitForm.input_LastFourlongtextbox(self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(4),6)    
                    
                    if lobname != "AOL":
                        BasicInfo.input_callrecordingnumber(call_Recording_Number_new)
                    '''Check13:1.The first two KPIs are in unchecked status.
                    ...
                    assert BasicInfo.get_anyCheckBoxStatus(5)==self.CheckBox_CheckedStatus
                    print SalesQuickHitForm.get_CallDateTime()
                    print DateandTimeofCall_selected_New
                    assert SalesQuickHitForm.get_CallDateTime()==DateandTimeofCall_selected_New
                    
                    #Step15.Click  button "Complete Coaching";
                    ...
                    #Total_coachnumber_database13=cursor.execute(sql_coach)  
                    '''Check13:3.Return to Coaching list page,Status Drop-down: shown'Incompleted' by default
                    ...
                    #Step3.14: From status drop-down, select Status ='Completed'->Click Filter;
                    ...
                    '''Check14:1.Check this coach is in completed coach list
                               2.Check the status of this coach is Completed'''
                    print SN_ThefirstCoachInCompletedlist
                    print SN_coachhomepage
                    assert SN_ThefirstCoachInCompletedlist==SN_coachhomepage
                    assert Status_ThefirstCoachInCompletedlist=="Completed"
                    
                    #Step3.15:Select this record enter  form detail page again.
                    #CoachHome.click_eachcoach(typename_path)
                    CoachHome.click_eachcoach(1)
                    '''Check15:1.Check the title of the detail page
                    ...
                    b=[2, 3, 4, 6, 7, 8, 10, 11, 12]
                    for buttonindex in b:   
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    b=[2, 3, 4, 5, 7, 8, 9, 11, 12] 
                    for buttonindex in b:
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        
                    b=[2, 3, 5, 6, 7, 9, 10, 11, 12]
                    for buttonindex in b:
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    b=[2, 3, 4]
                    for buttonindex in b:
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    b=[2,3]
                    for buttonindex in b:
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFiveStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    ...
                    b=[1, 2, 3, 4, 5, 6, 7 ]
                    for buttonindex in b:
                        assert SalesQuickHitForm.get_RadioButtonAtSectionSixStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_CheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionSixStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        assert SalesQuickHitForm.get_RadioButtonAtSectionSixStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    assert SalesQuickHitForm.get_FirstThreelongtextbox(4)==self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(4)
                    assert SalesQuickHitForm.get_FirstThreelongtextbox(5)==self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(5)
                    assert SalesQuickHitForm.get_FirstThreelongtextbox(6)==self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(6)
                    
                    assert SalesQuickHitForm.get_SecondThreelongtextbox(7, 5)==self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(7)
                    assert SalesQuickHitForm.get_SecondThreelongtextbox(8, 4)==self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(8)
                    assert SalesQuickHitForm.get_SecondThreelongtextbox(9, 8)==self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(9)
                    
                    assert SalesQuickHitForm.get_Seventhlongtextbox()==self.Longtextboxesprefix2+SalesQuickHitForm.get_FristSevenlongtextboxTitle(10)
                    
                    assert SalesQuickHitForm.get_LastFourlongtextbox(2)==self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(1)
                    assert SalesQuickHitForm.get_LastFourlongtextbox(3)==self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(1)
                    assert SalesQuickHitForm.get_LastFourlongtextbox(5)==self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(4)
                    assert SalesQuickHitForm.get_LastFourlongtextbox(6)==self.Longtextboxesprefix2+SalesQuickHitForm.get_LastTwolongtextboxTitle(4)   
                    ...
                    '''Check16:The  form cannot be edited now.( each field is in uneditable status)
                               ...
                               2.Input box of callRecordingID is disabled
                               3.Input box of DateandTimeofCall is disabled
                               4.Input box of commendation is disabled'''
                    ...
                    assert SalesQuickHitForm.CallDateTime_disabled() =='true'
                    assert SalesQuickHitForm.tactic_disabled()=='true'
                    b=[4,5,6]
                    for boxindex in b:
                        assert SalesQuickHitForm.FirstThreelongtextbox_disabled(boxindex)=='true'
                    assert SalesQuickHitForm.SecondThreelongtextbox_disabled(7, 5)=='true'
                    assert SalesQuickHitForm.SecondThreelongtextbox_disabled(8, 4)=='true'
                    assert SalesQuickHitForm.SecondThreelongtextbox_disabled(9, 8)=='true'
                    assert SalesQuickHitForm.Seventhlongtextbox_disabled()=='true'
                    b=[2, 3, 5, 6]
                    for boxindex in b:
                        assert SalesQuickHitForm.LastFourlongtextbox_disabled(boxindex)=='true'
                    ...
                    for buttonindex in b:
                        ...
                        SalesQuickHitForm.click_RadioButtonAtSectionOne(buttonindex,2)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionOne(buttonindex,3)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionOneStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    ...
                    for buttonindex in b:
                        ...
                        SalesQuickHitForm.click_RadioButtonAtSectionTwo(buttonindex,2)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionTwo(buttonindex,3)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionTwoStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    ...
                    for buttonindex in b:
                        ...
                        SalesQuickHitForm.click_RadioButtonAtSectionThree(buttonindex,2)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionThree(buttonindex,3)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionThreeStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    ...
                    for buttonindex in b:
                        ...
                        SalesQuickHitForm.click_RadioButtonAtSectionFour(buttonindex,2)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionFour(buttonindex,3)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFourStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    b=[2,3]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionFive(buttonindex,1)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFiveStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionFive(buttonindex,2)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFiveStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_CheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionFive(buttonindex,3)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionFiveStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus 
                    b=[1, 2, 3, 4, 5, 6, 7 ]
                    for buttonindex in b:
                        SalesQuickHitForm.click_RadioButtonAtSectionSix(buttonindex, 1)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionSixStatus(buttonindex,1)==self.ObservationReasonAndRadioButton_CheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionSix(buttonindex, 2)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionSixStatus(buttonindex,2)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                        SalesQuickHitForm.click_RadioButtonAtSectionSix(buttonindex, 3)
                        assert SalesQuickHitForm.get_RadioButtonAtSectionSixStatus(buttonindex,3)==self.ObservationReasonAndRadioButton_UnCheckedStatus
                    
                    
                    assert SalesQuickHitForm.FirstThreelongtextbox_disabled(4)=='true'
                    assert SalesQuickHitForm.FirstThreelongtextbox_disabled(5)=='true'
                    assert SalesQuickHitForm.FirstThreelongtextbox_disabled(6)=='true'
                    assert SalesQuickHitForm.SecondThreelongtextbox_disabled(7,5)=='true'
                    assert SalesQuickHitForm.SecondThreelongtextbox_disabled(8,4)=='true'
                    assert SalesQuickHitForm.SecondThreelongtextbox_disabled(9,8)=='true'
                    assert SalesQuickHitForm.Seventhlongtextbox_disabled()=='true'
                    assert SalesQuickHitForm.LastFourlongtextbox_disabled(2)=='true'
                    assert SalesQuickHitForm.LastFourlongtextbox_disabled(3)=='true'
                    assert SalesQuickHitForm.LastFourlongtextbox_disabled(5)=='true'
                    assert SalesQuickHitForm.LastFourlongtextbox_disabled(6)=='true'
                    
                    #Step3.17:Click BACK button form top left page.
                    Header.click_backbutton()
                    '''Check17:1.Back to search main page with status option 'Completed' is selected by default.
                               2.This coaching record is shown by default with status 'Completed'.
                               3.All completed coaching records listed.
                               4.On three databases servers: all info are the same.'''
                    ...
                    #Datalist_50=HMysql.Get_datafromDB(49, lobname, sitename, self.dbuser, self.dbpassword, sql_coach)
                    #Datalist_51=HMysql.Get_datafromDB(49, lobname, sitename, self.dbuser, self.dbpassword, sql_coach)
                    #Total_coachnumber_database17=cursor.execute(sql_coach)  
                    ...
                    #Step3.18:a.Logout and Login tablet with the Agent account that is coached by the  form(TL1/OM1/LC);
                    ...
                    CoachHome.click_eachcoach(1)
                    '''Check18:1.This  form record should be in the list
                               2. Enter the  form detail page with title 'Coaching - Commendation' On this form:1)Status = Completed; 2)Completed Date = The date when the form is completed;3)Acknowledged Date=blank
                    ...
                    #Step3.19:Click Back button at page bottom.
                    ...
                    #Step3.21:Click "Acknowledge Coaching" button;
                    ...
                    #Total_coachnumber_database21=cursor.execute(sql_coach)
                    '''Check21:1.Return to Coaching list page,Status Drop-down: Completed is selected by default.
                    ...
                    #Step3.22: 1.Status Drop-down:Select Status = Acknowledged;2.Click Filter;3.Open the  form again;4.Try to edit the  form.
                    CoachHome.select_status("Acknowledged")
                    CoachHome.click_filterbutton()
                    #CoachHome.click_eachcoach(SN_path)
                    CoachHome.click_eachcoach(1)
                    '''Check22:On this  form:
                    ...
                    #Step3.23:Click BACK button at page button.
                    ...
                    #Total_coachnumber_database23=cursor.execute(sql_coach)
                    '''Check23:Return to Coaching list page
                    ...
                    #Step3.24:1.Logout and Login tablet with TL1/OM1/LC account that added the  form;
                    ...
                    #CoachHome.click_eachcoach(SN_path)
                    CoachHome.click_eachcoach(1)
                    '''Check24:1.The  form is searched out with status shown 'acknowledged ';
