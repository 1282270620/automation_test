...
class CoachingMainPage_OM(unittest.TestCase):
    '''
    classdocs
    '''
    def setUp(self):
        #Case ID
        self.caseID="CoachingMainPage_OM"
    ...
    def test_CoachingMainPage(self):
        ...
        for i in range(0,len(self.testLOBSITE_list)):
            ...
            if Flag==True:
                ...
                for n in range(0,len(site_list)):
                    ...
                    OMInfo=GetLC.get_LCInfo(Adminurl,lobname, sitename, self.OMuserid, self.OMpassword, self.OMuserid)
                    OMname=OMInfo["Name"]
                    L=Login()
                    ...
                    sql_coach="select * from coach  where  assign_to_id="+self.OMuserid+" and classification=0 and status in (0,3) order by id desc"
                    ...
                    CT.Check_AllCoachName_withDB(TotalCoachName_page,hostindex, lobname_database, sitename_database, self.dbuser, self.dbpassword, sql_OMLC, sql_TL)
                    ...
                    SELECT MAX(active_time)\
                    FROM upload_history\
                    WHERE TYPE='2' AND DATE(data_date)<=now()) and r.hr_id not in (select hr_id from roster_teamleaders where history_id=(SELECT id\
                    FROM upload_history uh\
                    WHERE TYPE='2' AND uh.active_time=(\
                    SELECT MAX(active_time)\
                    FROM upload_history\
