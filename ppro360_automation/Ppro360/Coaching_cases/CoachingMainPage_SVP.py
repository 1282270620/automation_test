...
class CoachingMainPage_SVP(unittest.TestCase):
    '''
    classdocs
    '''
    def setUp(self):
        #Case ID
        self.caseID="CoachingMainPage_SVP"
    ...
    def test_CoachingMainPage_SVP(self):
        ...
        for i in range(0,len(self.testLOBSITE_list)):
            ...
            if Flag==True:
                ...
                for n in range(0,len(site_list)):
                    ...
                    #OMInfo=GetLC.get_LCInfo(self.adminurl,lobname, sitename, self.SVPuserid, self.OMpassword, self.OMuserid)
                    #OMname=OMInfo["Name"]
                    ...
                    sql_coach="select * from coach where classification=0 and status in (0,3) order by id desc"
                    ...
                    SELECT MAX(active_time) \
                    FROM upload_history \
                    WHERE TYPE='2' AND DATE(data_date)<='"+self.CurrentDate+"') and r.hr_id not in (select hr_id from roster_teamleaders where history_id=(SELECT id \
                    FROM upload_history uh \
                    WHERE TYPE='2' AND uh.active_time=(\
                    SELECT MAX(active_time) \
                    FROM upload_history \
                    WHERE TYPE='2' AND DATE(data_date)<='"+self.CurrentDate+"'))) order by r.firstname,r.lastname asc;"
