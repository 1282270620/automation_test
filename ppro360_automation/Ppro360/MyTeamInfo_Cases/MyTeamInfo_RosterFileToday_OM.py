...
class MyTeamInfo_RosterFileToday_OM(unittest.TestCase):


    def setUp(self):
        #Case ID
        self.caseID="MyTeamInfo_RosterFileToday_OM"
    ...
    def test_MyTeamInfo_RosterFileToday_OM(self):
        HMysql=HandleMySQL()
        L=Login()
        ...
        for i in range(0,len(self.testLOBSITE_list)):
            ...
            if Flag==True:
                ...
                for n in range(0,len(site_list)):
                    ...
                    OMNameInDB=HMysql.Get_datafromDB(hostindex, lobname_database, sitename_database, self.dbuser, self.dbpassword, sql_OM_name)
                    a=OMNameInDB[1][0]
                    OMName=a[0]+" "+a[1]
                    assert Hpage.get_HeaderTittle()==self.PageTitle1
                    assert Hpage.get_loginLob()==lobname
                    assert Hpage.get_loginSite()==sitename
                    assert Hpage.get_loginName()==OMName
                    ...
                    CheckTablet.Check_TabletHomepageCircle_OM(lobname)
                    ...
                    #click change password to enter chnage password page
                    Hpage.click_changePasswordLink()
                    print(Hpage.get_HeaderTittle())
                    ...
                    TLnumber=MTpage.get_TLnumber()
                    AllTLInfo_page=[]
                    for i in range(1,TLnumber+1):
                        MTpage.get_eachusername(i)
                        MTpage.get_eachhrid(i)
                        TLinfo=[MTpage.get_eachusername(i),MTpage.get_eachhrid(i)]
                        AllTLInfo_page.append(TLinfo)
                    
                    sql_TL_list="SELECT rt.hr_id tl_hr_id,rtl.firstname tl_first_name,rtl.lastname tl_last_name,rt.team_id as rt_teamID,r.hr_id as agentHRID,rag.firstname as agentFirstName,rag.lastname as agentLastName FROM roster_teamleaders rt JOIN roster rtl ON rt.history_id = rtl.history_id AND rt.hr_id = rtl.hr_id LEFT JOIN roster r ON rt.history_id=r.history_id AND rt.team_id=r.team_id JOIN roster rag ON r.history_id = rag.history_id AND r.hr_id = rag.hr_id WHERE rt.history_id=(SELECT id FROM upload_history uh WHERE TYPE='2' AND uh.active_time=(SELECT MAX(active_time) FROM upload_history WHERE TYPE='2' AND DATE(data_date)<="+"'"+currentdate+"')) and rt.hr_id=r.hr_id;"
                    TLlistInDB=HMysql.Get_datafromDB(hostindex, lobname_database, sitename_database, self.dbuser, self.dbpassword, sql_TL_list)
                    Tllist=TLlistInDB[1]
                    AllTLInfo_DB=[]
                    for i in range(0,len(Tllist)):
                        TL_name=Tllist[i][1]+" "+Tllist[i][2]
                        TL_hrid=Tllist[i][0]
                        TL_info=[TL_name,TL_hrid]
                        AllTLInfo_DB.append(TL_info)
                    
                    print(AllTLInfo_page)
                    print(AllTLInfo_DB) 
                    assert len(AllTLInfo_page)==len(AllTLInfo_DB)
                    for item in AllTLInfo_page:
                        assert item in AllTLInfo_DB
                    ...
                    for i in range(i,TotaluserNumber+1):
                        TL_hrid_excel=GetMyTeamContent.Get_HRID(filename_prefix, sheetname)
                    print('tl hrid:', TL_hrid_excel)
                    
                    OneUser=[]
                    for name in TL_hrid_excel:
                        OneUser=[name, TL_hrid_excel[name]]
                        AllTLInfo_excel.append(OneUser)
                    print(AllTLInfo_excel)
                    assert len(AllTLInfo_page)==len(AllTLInfo_excel)
                    for item in AllTLInfo_page:
                        assert item in AllTLInfo_excel                   
