...
from CoachingAndTriadCoaching_Pages.DDDMProcessConfirmationPage import DDDMProcessConfirmationPage
from Tablet_pages.HeaderPage import HeaderPage
from public_method.Get_AllRoleAccountForTest import Get_AllRoleAccountForTest

class DDDMProcessConfirmation_PlannedToCompleted(unittest.TestCase):


    def setUp(self):
        #Case ID
        self.caseID="DDDMProcessConfirmation_PlannedToCompleted"
        ...
        self.coachformname="Data Drive Decision Making (DDDM) Process Confirmation" 
    ...
    def test_DDDMProcessConfirmation_PlannedToCompleted(self):
        ...
        Header=HeaderPage()
        DDDMProcessConfirmationForm=DDDMProcessConfirmationPage()
        ...
        for i in range(0,len(self.testLOBSITE_list)):
            ...
            if Flag==True:
                ...
                for n in range(0,len(site_list)):
                    ...
                    userid=DDDMProcessConfirmationForm.get_firstTL_HRID(1)
                    L.logout_tablet()
                    ...
                    for i in range(2,7):
                        assert DDDMProcessConfirmationForm.BodyComments_disabled(i)=='true'
                        assert DDDMProcessConfirmationForm.GradeComments_disabled(i)=='true'
                    
                    
                    for i1 in range(3,6):
                        assert DDDMProcessConfirmationForm.LastComments_disabled(i1)=='true'
                    #assert DDDMProcessConfirmationForm.get_OverallScoreColor()==self.OneColor
                    assert DDDMProcessConfirmationForm.OverallScore_disabled()=='true'
                    assert DDDMProcessConfirmationForm.get_OverallScoreComments()=='0'
                    ...
                    for j in range(2,7):
                        DDDMProcessConfirmationForm.input_BodyComments(j, (self.Longtextboxesprefix1+DDDMProcessConfirmationForm.get_BodyTitle(j)))
                    num1='1'
                    num2='2'
                    num3='3'
                    DDDMProcessConfirmationForm.input_GradeComments(3,num3)
                    DDDMProcessConfirmationForm.input_GradeComments(4,num2)
                    DDDMProcessConfirmationForm.input_GradeComments(5,num1)
                    DDDMProcessConfirmationForm.input_GradeComments(6,num1)
                    for j1 in range(3,6):
                        DDDMProcessConfirmationForm.input_LastComments(j1, (self.Longtextboxesprefix1+DDDMProcessConfirmationForm.get_LastTitle(j1)))
                    ...
                    for k in range(2,7):
                        assert DDDMProcessConfirmationForm.get_BodyComments(k)==self.Longtextboxesprefix1+DDDMProcessConfirmationForm.get_BodyTitle(k)
                    for k1 in range(3,6):
                        assert DDDMProcessConfirmationForm.get_LastComments(k1)==self.Longtextboxesprefix1+DDDMProcessConfirmationForm.get_LastTitle(k1)
                    
                    assert DDDMProcessConfirmationForm.get_GradeComments(2)==''
                    assert DDDMProcessConfirmationForm.get_GradeComments(3)==num3
                    assert DDDMProcessConfirmationForm.get_GradeComments(4)==num2
                    assert DDDMProcessConfirmationForm.get_GradeComments(5)==num1
                    assert DDDMProcessConfirmationForm.get_GradeComments(6)==num1
                    assert DDDMProcessConfirmationForm.get_GradeColor(3)==self.ThreeColor
                    assert DDDMProcessConfirmationForm.get_GradeColor(4)==self.TwoColor
                    assert DDDMProcessConfirmationForm.get_GradeColor(5)==self.OneColor
                    assert DDDMProcessConfirmationForm.get_GradeColor(6)==self.OneColor
                    assert DDDMProcessConfirmationForm.get_OverallScoreComments()=='1.75'
                    assert DDDMProcessConfirmationForm.get_OverallScoreColor()==self.OneColor
